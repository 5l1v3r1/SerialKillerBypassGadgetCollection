...
/**
 * Created by alvaro on 08/04/16. Basically taken from YSOSerial:
 * https://raw.githubusercontent.com/frohoff/ysoserial/master/src/main/java/ysoserial/GeneratePayload.java
 */
public class Main {
    ...
    private static void printUsage() {
        ...
        System.err.println(
                "Usage: java -jar serialkiller-bypass-gadgets-[version]-all.jar [payload type] [bypass type] '[command to execute]'");
        ...
        // Collections.sort(payloadClasses, new GeneratePayload.ToStringComparator());
        // // alphabetize
        for (Class<? extends ObjectPayload> payloadClass : payloadClasses) {
            System.err.println("\t\t" + payloadClass.getSimpleName() + " "
                    + Arrays.asList(Dependencies.Utils.getDependencies(payloadClass)));
        }

        System.err.println("\n\tAvailable bypass types:");
        final List<Class<? extends BypassGadget>> bypassClasses = new ArrayList<Class<? extends BypassGadget>>(
                BypassGadget.getBypassGadgets());
        // Collections.sort(bypassClasses, new GeneratePayload.ToStringComparator()); //
        // alphabetize
        for (Class<? extends BypassGadget> bypassClass : bypassClasses) {
            System.err.println("\t\t" + bypassClass.getSimpleName() + " "
                    + Arrays.asList(Dependencies.Utils.getDependencies(bypassClass)));
        }
